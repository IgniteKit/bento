...
class WordPressBundler {
    ...
    async processSass(inputPath, outputDir, relativePath) {
        try {
            ...
            // Compile SCSS to CSS
            const result = execSync(`npx sass "${inputPath}" --style=expanded --no-source-map`, {
                encoding: 'utf8',
    ...
    async transpileJS(content) {
        try {
            ...
            const result = execSync(`echo '${content.replace(/'/g, "\\'")}' | npx babel --presets=@babel/preset-env`, {
                encoding: 'utf8',
                stdio: ['pipe', 'pipe', 'pipe']
            });
            return result;
    ...
    async minifyJS(content) {
        try {
            ...
            console.warn('⚠️  JS minification failed, using original content');
            return content;
        }
    }
    ...
    async installDependencies() {
        if (this.dependencies.size > 0) {
            ...
            try {
                execSync(`npm install ${Array.from(this.dependencies).join(' ')}`, { stdio: 'inherit' });
            } catch (error) {
